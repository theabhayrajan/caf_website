    ...
    return (
        <>
            ...
            >
                ...
                {/* --- Navigation Bar --- */}
                <div
                    className={`bg-white transition-all duration-300 ${isSticky ? "" : ""
                        }`}
                >
                            ...
                            {links.map((link) => {
                                ...
                                return (
                                    <Link
                                        key={link.name}
                                        href={link.href}
                                        className={`relative flex items-center h-full transition-colors duration-200 ${isActive ? "text-[#2E98DA]" : "text-gray-800"
                                            } hover:text-[#2E98DA] group`}
                                    >
                                        <span className="pb-1">{link.name}</span>
                                        <span
                                            className={`absolute left-0 bottom-0 h-[2px] bg-[#2E98DA] transition-all duration-300 ${isActive ? "w-full" : "w-0 group-hover:w-full"
                                                }`}
                                        />
                                    </Link>
                                );
                    ...
                    {/* --- Mobile Menu --- */}
                    <div
                        className={`md:hidden bg-white shadow-md transition-max-height duration-300 overflow-hidden ${mobileOpen ? "max-h-[400px]" : "max-h-0"
                            }`}
                    >
                                ...
                                {links.map((link) => {
                                    ...
                                    return (
                                        <Link
                                            ...
                                            className={`block px-3 py-2 rounded font-medium ${isActive ? "text-[#2E98DA]" : "text-gray-800"
                                                }`}
                                        >
                                            {link.name}
                                        </Link>
